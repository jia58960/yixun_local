...
switch($action){
	case "get":
		if (!empty ($_POST['id'])) {
			...
			$con = @mysql_connect("localhost","root","krit1979") or die('error!');

			mysql_select_db("kr_ccms", $con);
	...
	case "insert":
		...
		$con = @ mysql_connect("localhost","root","krit1979") or die('ddderror!');//http://10.188.199.8
		
		mysql_select_db("kr_ccms", $con) or die('选择数据库失败!');
