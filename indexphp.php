...
?>
<input type="hidden" name="ipvalidate" id="ipvalidate" value=
<?php 
if(strpos(get_client_ip(), '10.188') !== false){
	echo "http://10.188.199.8:19998/yixun_local/";			
	}else{		
	echo "http://szkingray.3322.org:19998/yixun_local/";
}
?>>
<input type="hidden" name="ipvalidatejava" id="ipvalidatejava" value=
<?php 
if(strpos(get_client_ip(), '10.188') !== false){
	echo "http://10.188.192.200:8080/YIXUN_1.5_EE/";			
	}else{		
	echo "http://szkingray.3322.org:18080/YIXUN_1.5_EE/";
}
?>>
<!--IP verification START-->
<script type="text/javascript">
	var io = document.getElementById('ipvalidate').value;
	var io_java = document.getElementById('ipvalidatejava').value;
</script>
<!--IP verification END-->
...
<body class="page-header-fixed">
	...
	<!-- <script src="media/js/jquery.history.js" type="text/javascript"></script> -->
	<script type="text/javascript" src="media/js/crypt.js"></script>

	<script src="media/configphp.js"></script>

	<script type="text/javascript">
		//菜单
		jQuery(document).ready(function() {
		//All of the modules have to load!
        ...
        //加载主模块
		/*if(!urljasonfix.dashboard){
        	urljasonfix.dashboard =
			[{
			"ra":"dashboard",
			"dir":"Index/index",
			"dif":'',
        ...
        yixunload(urljasonfix.dashboard,urlfix);*/

        //加载子菜单
	    /*if(!urljasonfix.sub_m){
        	urljasonfix.sub_m =
			[{
			"ra":"sub_m",
			"dir":"Com/Submenu",
			"dif":'resource/listSubResourcesByType',
			"p":{"resourceId":"4","resourceTypeId":1},
			"p2":{"subid":""}
			}];
        }
        yixunload(urljasonfix.sub_m,urlfix);*/

        //点击首页左侧二级菜单
	    $('.page-sidebar').on('click', ' li > a.open_tabs', function (e) {
                 //@@后面还需要做点击高亮，同时更改urlJSON数据
                e.preventDefault();
                App.scrollTop();
                ...
                //return;
                var urlfixObj = eval(urlfix);
                
                urlfixObj.Db_m[0].p2.fid = link_id;             
                flushurl();
                ...
                yixunload(submenu,urlfix);
               	//App.unblockUI($("#sub_m"),false);				
		...
		var menuflag = urljasonfix.Db_m[0].p2.fid; //获取主菜单ID
